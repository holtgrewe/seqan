...
/*				# Pick a library size
    ...
    while (readCounter < numReads)
    {
        ...
        if (revComp)
            reverseComplement(readTemplate);
...
/*			int start1 = startPos;
            ...
            if (!kickOut)
            {
                ...
                ++readCounter;
                if (verbose && readCounter % 10000 == 0)
                    std::cout << readCounter << "..." << std::flush;
                //Add read to readSet
                if (!revComp)
...
//	} else {
    if (verbose)
    {
        ...
        {
            if ((kickOutCount[i] + bucketCounter[i]) > 0)
                std::cout << (double) kickOutCount[i] / (kickOutCount[i] + bucketCounter[i]) << std::endl;
            else
                std::cout << "0\n";
        }
        std::cout << std::endl;
