...
{
//IOREV do other things deal with Prb-Files, should this be moved to statistics?
...
//IOREV
    ...
    if (*pm_options.shapeFile != 0)
    {
        ...
        if (result == 0)
            std::cerr << "0 shapes parsed." << std::endl;
        else if (pm_options.verbose)
            std::cerr << result << " shapes parsed." << std::endl;
        filestrm.close();
    }
    ...
    {
        if (length(shapeStrings[i]) > totalN)
            continue;
...
//			computeFilteringLoss(found, states, length(shapeStrings[i]), maxT, maxErrors,  logErrorDistribution);
        ...
        for (unsigned e = minErrors; e < maxErrors; ++e)
        {
            ...
            for (unsigned t = maxT - 1; t > minT; --t)
            {
                ...
                if (lossrate <= 0.0)
                {
                    if (highestOptimalFound)
                        break;
                    else
                        highestOptimalFound = true;
                }
                if (lossrate > 0.2)
                    continue;
...
extrapolateNK(TOptions & pm_options)
{
    ...
    int bestN, currN;
    if (pm_options.optionHammingOnly)
        bestN = pm_options.maxComputedHammingN;
    else
        bestN = pm_options.maxComputedEditN;
    currN = bestN;
...
chooseParams(RazerSOptions<TSpec> & r_options, ParamChooserOptions & pm_options)
{
    ...
    if (r_options.maqMapping)
    {
        ...
        if (r_options.artSeedLength == 28)
            r_options.shape = "1101001110100111010011";                            //q13
        if (r_options.artSeedLength == 29)
            r_options.shape = "11101001110100111010011";                            //q14
        if (r_options.artSeedLength == 30)
            r_options.shape = "111010011101001110100101";                            //q14
        if (r_options.artSeedLength == 31)
            r_options.shape = "111010011101001110100101";                            //q14
        if (r_options.lowMemory && r_options.artSeedLength == 29)
            r_options.shape = "10101001110100111010011";
...
//			std::cerr << "\nNo session id given, using prefix 'userdef'"<<std::endl;
    ...
    else if (!pm_options.prefixCount)
        pm_options.fprefix[0] = "results";
    ...
    if (pm_options.verbose)
    {
        ...
        if (pm_options.extrapolate)
            std::cerr << "Extrapolating from read length " << pm_options.extrapolN << " and " << pm_options.extrapolK << "errors." << std::endl;
    }
    ...
    if (pm_options.verbose)
        std::cout << std::endl << " Choose " << std::endl << "shape: " << r_options.shape << std::endl << " and " << std::endl << "threshold: " << r_options.threshold << std::endl << " to achieve optimal performance for expected recognition rate >= " << (100.0 - 100.0 * pm_options.optionLossRate) << "% (expected recognition = " << (100.0 - pm_options.chosenLossRate * 100.0) << "%)" << std::endl << std::endl;
